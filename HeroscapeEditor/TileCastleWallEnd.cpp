// TileCastleWallEnd.cpp : implementation file
//
...
// The constructor
//
CTileCastleWallEnd::CTileCastleWallEnd()
{
	m_NbTile = 1;
	m_TileHeight = TILE3D_ZHEIGHT;
	m_Type = TYPE_CASTLE*1000+203;
...
//
CTile* CTileCastleWallEnd::GetCopy()
{
...
// Draw the tiles
//
void CTileCastleWallEnd::Draw( CDC* pDC, int x, int y, int x2, int x3, int y2, int y3, int y4, int DrawingLevel)
{
...
// Draw the tiles
//
void CTileCastleWallEnd::DrawSurround( CDC* pDC, int x, int y, int x2, int x3, int y2, int y3, int y4, int DrawingLevel)
{
	// Compute polyline around tiles
	POINT	Points1[13];
	int		NbPoint1 = 13;
	...
	else
	{
		...
		Points1[6].y=y+y3-y2;
		Points1[7].x=(int) (x+x2/2.0);
		Points1[7].y=(int) (y+y3-y2+y2*0.3);
		Points1[8].x=(int) (x+x2+x2/2.0);
		Points1[8].y=(int) (y+y3-y2+y2*0.3);
		Points1[9].x=(int) (x+x2+x2*0.8);
		Points1[9].y=(int) (y+(y3-y2)/2.0);
		Points1[10].x=(int) (x+x2+x2/2.0);
		Points1[10].y=(int) (y-y2*0.3);
		Points1[11].x=(int) (x+x2/2.0);
		Points1[11].y=(int) (y-y2*0.3);
		Points1[12].x=(int) (x+x2/2.0);
		Points1[12].y=y;
	}
...
// Draw more on a tile
//
void CTileCastleWallEnd::DrawExtend( CDC* pDC, int x, int y, int x2, int x3, int y2, int y3, int y4, int DrawingLevel)
{
	DrawSurround( pDC, x, y, x2, x3, y2, y3, y4, DrawingLevel);
...
//
void CTileCastleWallEnd::Render1( void* pView, bool ForShadow, bool BindTexture )
{
	...
	((CHeroscapeEditor3DView*) pView)->m_pCastleWallEnd->Render( BindTexture );
...
//
void CTileCastleWallEnd::VerifySomethingUnderTile( void* pTheDoc )
{
}

// Fill the members with members of an other tile
//
void CTileCastleWallEnd::FillCopy( CTile* pTile )
{
	CTile::FillCopy( pTile );
	m_TileColor2 = ((CTileCastleWallEnd*) pTile)->m_TileColor2;
}
