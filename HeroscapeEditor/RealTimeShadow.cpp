...
{
  TessellationContext *context = (TessellationContext*) polyData;
  GLfloat *newHolder;
...
generateSilhouette(TessellationContext * context)
{
  ...
  if (context->combineNext > context->combineListSize) {
    ...
    context->combineList = (GLfloat*) SHARED_MALLOC(sizeof(GLfloat) * 2 * context->combineListSize);
  ...
  if (context->excessList3D) {
    ...
    svs->silhouette = (GLfloat*) SHARED_REALLOC(svs->silhouette,
...
printf("numProcessors = %d\n", numProcessors);
  ...
  scene->sceneAmbient[0] = (GLfloat) 0.2;
  scene->sceneAmbient[1] = (GLfloat) 0.2;
  scene->sceneAmbient[2] = (GLfloat) 0.2;
  scene->sceneAmbient[3] = (GLfloat) 1.0;
