// TileFigure.cpp : implementation file
//
...
// Delete smoke dice
//
void CTileFigure::DeleteSmokeDice()
{
	for( int i=0; i<100; i++ )
...
//
CTile* CTileFigure::GetCopy()
{
	CTileFigure* pTile = new CTileFigure;
...
// Render the tile
//
void CTileFigure::Render1( void* pView, bool ForShadow, bool BindTexture )
{
	...
	if( m_PosY%2==1 )
	{
		if( m_TilePosition[0].y%2==0 )
			glTranslated( (m_PosX+m_TilePosition[0].x)*TILE3D_WIDTH+TILE3D_X3, -(m_PosY+m_TilePosition[0].y)*TILE3D_HEIGHT, m_PosZ*TILE3D_ZHEIGHT );
		else
			glTranslated( (m_PosX+m_TilePosition[0].x+1)*TILE3D_WIDTH, -(m_PosY+m_TilePosition[0].y)*TILE3D_HEIGHT, m_PosZ*TILE3D_ZHEIGHT );
	}
	else
	{
		if( m_TilePosition[0].y%2==1 )
			...
			glTranslated( (m_PosX+m_TilePosition[0].x)*TILE3D_WIDTH, -(m_PosY+m_TilePosition[0].y)*TILE3D_HEIGHT, m_PosZ*TILE3D_ZHEIGHT );
	}
	...
	if( pObject3D!=NULL )
	{
		...
		if( m_CreationTime>=0 && m_CreationTime<0.7 )
		{
			...
			DeleteSmokeDice();
			double Size = min(1,fabs(sin(m_CreationTime/0.5*PI*8.5/10*4)));
...
// Draw more on a tile
//
void CTileFigure::DrawExtendTile( CDC* pDC, int x, int y, int x2, int x3, int y2, int y3, int y4, int DrawingLevel)
{
...
//
void CTileFigure::LoadNecessaryObjects( void* pView )
{
...
//
bool CTileFigure::IsGameAvailable()
{
	return false;
}
...
//
void CTileFigure::GenerateSmokeForExplode( CHeroscapeEditor3DView* pView )
{
	int SmokeNumber = 0;
...
//
void CTileFigure::GenerateSmokeForDice( CHeroscapeEditor3DView* pView, int DiceIndex )
{
	int SmokeNumber = 10;
...
// Delete smoke object
//
void CTileFigure::DeleteSmokeObjects()
{
...
// Render explode
//
void CTileFigure::RenderExplode( void* pView, bool BindTexture )
{
	if( m_ExplodeTime>0.5 && m_ExplodeTime<1 )
	{
...
// Render explode dice
//
void CTileFigure::RenderExplodeDice( void* pView, bool BindTexture, int DiceIndex, double Time )
{
	if( Time>1 )
		return;
