...
void CConnexionSocket::ReadAMessage( CString IP, int Port )
{
	...
	case SMI_TOSERVER_SEARCH_SERVER:
		TRACE( _T("Receive search server\n") );
		CreateBuffer();
		WriteType(theApp.m_GameName);
		WriteType(theApp.m_GamePort);
		SendMessageTo(SMI_TOCLIENT_GAME_NAME,Port,IP);
		break;
	default:
		{
			char a;
...
{
	// doesn't accept if the game is running
	if( theApp.m_pGameDocServer!=NULL )
	{
		if( theApp.m_pGameDocServer->m_CurrentStep!=CGameDocServer::STEP_CONNEXION )
			return;
	}
	...
	if( theApp.m_pGameDoc!=NULL )
	{
		...
		if( Accept( *pPlayer->m_pToClientSocket )==0 )
			SocketError();
		pPlayer->m_pToClientSocket->AsyncSelect();
		theApp.m_pGameDocServer->AddPlayer( pPlayer );
		pPlayer->m_pToClientSocket->CreateBuffer();
		pPlayer->m_pToClientSocket->SendMessage(SMI_TOCLIENT_GET_PLAYER_NAME);
	}
