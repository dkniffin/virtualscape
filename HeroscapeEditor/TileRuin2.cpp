// TileRuin2.cpp : implementation file
//
...
//
void CTileRuin2::OnRotationChange()
{
	int CurrentPos = 0;
...
// Draw the tiles
//
void CTileRuin2::Draw( CDC* pDC, int x, int y, int x2, int x3, int y2, int y3, int y4, int DrawingLevel)
{
	...
	case 4:
		...
		Points[3].y=y+y3;
		Points[4].x=x+x3+x2;
		Points[4].y=y+y3-y2+y3;
		Points[5].x=x+x2+x2;
		Points[5].y=y+y3+y3;
		break;
	case 5:
		Points[0].x=x+x3+x2-x2;
		Points[0].y=y-y3+y3;
		Points[1].x=x+x3+x2-x2;
		Points[1].y=y-y2+y3;
		Points[2].x=x+x3-x2;
		Points[2].y=y+y3;
		Points[3].x=x+x3-x2;
		Points[3].y=y+y3-y2+y3;
		Points[4].x=x+x2-x2;
		Points[4].y=y+y3+y3;
		Points[5].x=x-x2;
		Points[5].y=y+y3-y2+y3;
		break;
	}
...
//
void CTileRuin2::Render1( void* pView, bool ForShadow, bool BindTexture )
{
	...
	case 3:
		...
		break;
	case 4:
		glTranslated( TILE3D_X3, -TILE3D_HEIGHT, 0 );
		glRotated( -240, 0, 0, 1 );
		break;
	case 5:
		glTranslated( TILE3D_X1, -TILE3D_HEIGHT, 0 );
		glRotated( -300, 0, 0, 1 );
		break;
	}

	((CHeroscapeEditor3DView*) pView)->m_pRuin2->Render( BindTexture );
