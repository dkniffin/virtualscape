// TileGlacier4.cpp : implementation file
//
...
//
void CTileGlacier4::OnRotationChange()
{
	int CurrentPos = 0;
...
// Draw more on a tile
//
void CTileGlacier4::DrawExtend( CDC* pDC, int x, int y, int x2, int x3, int y2, int y3, int y4, int DrawingLevel)
{
	CFont Font;
	...
	pDC->DrawText( _T("7"), CRect(Point1.x-x3,Point1.y-x3,Point1.x+x3+1,Point1.y+x3+1), DT_CENTER | DT_VCENTER | DT_SINGLELINE );
	pDC->DrawText( _T("11"), CRect(Point2.x-x3,Point2.y-x3,Point2.x+x3+1,Point2.y+x3+1), DT_CENTER | DT_VCENTER | DT_SINGLELINE );
	pDC->DrawText( _T("11"), CRect(Point3.x-x3,Point3.y-x3,Point3.x+x3+1,Point3.y+x3+1), DT_CENTER | DT_VCENTER | DT_SINGLELINE );
	pDC->DrawText( _T("9"), CRect(Point4.x-x3,Point4.y-x3,Point4.x+x3+1,Point4.y+x3+1), DT_CENTER | DT_VCENTER | DT_SINGLELINE );
...
//
void CTileGlacier4::Render1( void* pView, bool ForShadow, bool BindTexture )
{
	...
	case 4:
		...
		break;
	case 5:
		glTranslated( -TILE3D_WIDTH*0.5, -TILE3D_Y1*2+TILE3D_Y2, 0 );
		glRotated( -300, 0, 0, 1 );
		break;
	}

	((CHeroscapeEditor3DView*) pView)->m_pGlacier4->Render( BindTexture );
