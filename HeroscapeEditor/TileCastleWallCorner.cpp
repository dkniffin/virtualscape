// TileCastleWallCorner.cpp : implementation file
//
...
// The constructor
//
CTileCastleWallCorner::CTileCastleWallCorner()
{
	m_NbTile = 1;
	m_TileHeight = TILE3D_ZHEIGHT;
	m_Type = TYPE_CASTLE*1000+201;
...
//
CTile* CTileCastleWallCorner::GetCopy()
{
...
// Draw the tiles
//
void CTileCastleWallCorner::Draw( CDC* pDC, int x, int y, int x2, int x3, int y2, int y3, int y4, int DrawingLevel)
{
...
// Draw the tiles
//
void CTileCastleWallCorner::DrawSurround( CDC* pDC, int x, int y, int x2, int x3, int y2, int y3, int y4, int DrawingLevel)
{
	// Compute polyline around tiles
	POINT	Points1[9];
	POINT	Points2[5];
	...
	Points1[3].x=x+x2;
	Points1[3].y=y+y3;
	Points1[4].x=(int) (x+x2+x2/5.0);
	Points1[4].y=(int) (y+y3-y2*0.7);
	Points1[5].x=(int) (x+x2+x2/2.0);
	Points1[5].y=(int) (y+y3-y2/1.5);
	Points1[6].x=(int) (x+x3/1.05);
	Points1[6].y=(int) (y+(y3-y2)/2.0);
	Points1[7].x=(int) (x+x2+x2/2.0);
	Points1[7].y=(int) (y-y2/2.5+1);
	Points1[8].x=(int) (x+x2+x2/5.0);
	Points1[8].y=(int) (y-y2*0.3+1);

	Points2[0].x=x;
	Points2[0].y=y;
	Points2[1].x=x;
	Points2[1].y=y+y3-y2;
	Points2[2].x=(int) (x+x2/3.0);
	Points2[2].y=(int) (y+(y3-y2)/4.0*3);
	Points2[3].x=(int) (x+x2/5.0);
	Points2[3].y=(int) (y+(y3-y2)/2.0);
	Points2[4].x=(int) (x+x2/3.0);
	Points2[4].y=(int) (y+(y3-y2)/4.0);
...
// Draw more on a tile
//
void CTileCastleWallCorner::DrawExtend( CDC* pDC, int x, int y, int x2, int x3, int y2, int y3, int y4, int DrawingLevel)
{
	CFont Font;
	...
	pDC->DrawText( _T("9"), CRect(CenterX-x3,CenterY-x3,CenterX+x3,CenterY+x3), DT_CENTER | DT_VCENTER | DT_SINGLELINE );
...
//
void CTileCastleWallCorner::Render1( void* pView, bool ForShadow, bool BindTexture )
{
	...
	((CHeroscapeEditor3DView*) pView)->m_pCastleWallCorner->Render( BindTexture );
...
//
void CTileCastleWallCorner::VerifySomethingUnderTile( void* pTheDoc )
{
}

// Fill the members with members of an other tile
//
void CTileCastleWallCorner::FillCopy( CTile* pTile )
{
	CTile::FillCopy( pTile );
	m_TileColor2 = ((CTileCastleWallCorner*) pTile)->m_TileColor2;
}
